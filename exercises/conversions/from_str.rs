...
impl FromStr for Person {
    ...
    fn from_str(s: &str) -> Result<Person, Self::Err> {
        // Step 1: If the length of the provided string is 0, return an error.
        if s.is_empty() {
        ...
        // Step 6: If while extracting the name and the age something goes wrong, return an error.
        if name.is_empty() {
            return Err(ParsePersonError::NoName);
        }

        // If everything goes well, return a Result with a Person object.
        Ok(Person { name, age })
    }
}
